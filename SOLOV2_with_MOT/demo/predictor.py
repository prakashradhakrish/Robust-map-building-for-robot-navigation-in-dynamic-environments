...
class VisualizationDemo(object):
    ...
    def run_on_image(self, image):
        ...
        predictions,feat,maskfeat = self.predictor(image)
        #predictions = self.extractcustomclasses(predictions,image)
        # Convert image from OpenCV BGR format to Matplotlib RGB format.
        image = image[:, :, ::-1]
        #print(maskfeat['maskfeatures'].shape)
        image=np.array(image)
        image[:,:,:] = 255
        pred = (predictions['instances'].pred_masks).cpu()
        #print(pred)
        #pred = np.asarray(pred[0]).astype("uint8")
        #cv2.imwrite('mask_cv.png',pred)

        self.vis_text = False
        self.instance_mode = ColorMode.IMAGE_BW
        if self.vis_text:
    ...
    def extractcustomclasses(self,outputs,image):
